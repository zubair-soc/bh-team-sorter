...
const HockeyTeamBalancerWithUpload = () => {
  ...
  // Reset newGroup when switching classes
  useEffect(() => { setNewGroup([]); setUploadError(''); }, [selectedClassId]);
  ...
  const handleFileUpload = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        alert(`Loaded ${newPlayers.length} players (${newPlayers.filter(p => p.isGoalie).length} goalies) and ${newFriendGroups.length} friend groups into ${selectedClass?.name || 'this class'}!`);
  ...
  const downloadForExcel = () => {
    ...
    t1.forEach(p => { const gi = friendGroups.findIndex(g => g.includes(p.id)); content += `${p.name||'Unknown'},${p.rating},${p.preferredSize},${p.assignedSize||'TBD'},${p.isGoalie?'Goalie':'Skater'},${p.isIR?'IR':'Active'},${gi>=0?`Group ${gi+1}`:''}\n`; });
    content += `\n================================================================================\nTEAM 2: ${teamNames.team2.toUpperCase()}\n================================================================================\nName,Rating,Preferred Size,Assigned Size,Position,IR Status,Friend Group\n`;
    t2.forEach(p => { const gi = friendGroups.findIndex(g => g.includes(p.id)); content += `${p.name||'Unknown'},${p.rating},${p.preferredSize},${p.assignedSize||'TBD'},${p.isGoalie?'Goalie':'Skater'},${p.isIR?'IR':'Active'},${gi>=0?`Group ${gi+1}`:''}\n`; });
    const a = document.createElement('a');
  ...
  const downloadAdminRoster = () => {
    ...
    t1.forEach(p => { const gi = friendGroups.findIndex(g => g.includes(p.id)); content += `\n# | ${p.name||'Unknown'} | Rating: ${p.rating}\n  Position: ${p.isGoalie?'Goalie':'Skater'}\n  Jersey: ${p.preferredSize} → ${p.assignedSize||'TBD'}\n  Status: ${p.isIR?'IR':'Active'}${p.isWoman?' | Woman':''}\n  ${gi>=0?`Friend Group ${gi+1}`:'No Group'}\n`; });
    content += `\nTotal: ${t1.length} | Rating: ${stats.team1.rating} | Avg: ${stats.team1.count>0?(stats.team1.rating/stats.team1.count).toFixed(2):0}\n\n================================================================================\nTEAM 2: ${teamNames.team2.toUpperCase()}\n================================================================================\n`;
    t2.forEach(p => { const gi = friendGroups.findIndex(g => g.includes(p.id)); content += `\n# | ${p.name||'Unknown'} | Rating: ${p.rating}\n  Position: ${p.isGoalie?'Goalie':'Skater'}\n  Jersey: ${p.preferredSize} → ${p.assignedSize||'TBD'}\n  Status: ${p.isIR?'IR':'Active'}${p.isWoman?' | Woman':''}\n  ${gi>=0?`Friend Group ${gi+1}`:'No Group'}\n`; });
    content += `\nTotal: ${t2.length} | Rating: ${stats.team2.rating} | Avg: ${stats.team2.count>0?(stats.team2.rating/stats.team2.count).toFixed(2):0}\n`;
    const a = document.createElement('a');
    a.href = `data:text/plain;base64,${btoa(unescape(encodeURIComponent(content)))}`;
  ...
  const downloadPublicRoster = () => {
    ...
    t1.forEach(p => { content += `# | ${p.name||'Unknown'} - ${p.isGoalie?'Goalie':'Skater'}${p.isIR?' (IR)':''}\n`; });
    content += `\nTotal: ${t1.length} | Goalies: ${t1.filter(p=>p.isGoalie).length}\n\n================================================================================\nTEAM 2: ${teamNames.team2.toUpperCase()}\n================================================================================\n`;
    t2.forEach(p => { content += `# | ${p.name||'Unknown'} - ${p.isGoalie?'Goalie':'Skater'}${p.isIR?' (IR)':''}\n`; });
    content += `\nTotal: ${t2.length} | Goalies: ${t2.filter(p=>p.isGoalie).length}\n`;
    const a = document.createElement('a');
    a.href = `data:text/plain;base64,${btoa(unescape(encodeURIComponent(content)))}`;
  ...
  const balanceTeams = () => {
    ...
    const ratingOf = (team) => Object.entries(assignments).filter(([,t]) => t === team).reduce((s,[id]) => s + players.find(p => p.id === parseInt(id)).rating, 0);
    const womenOf  = (team) => Object.entries(assignments).filter(([,t]) => t === team).reduce((s,[id]) => s + (players.find(p => p.id === parseInt(id)).isWoman ? 1 : 0), 0);

    // Friend groups
    const groups = friendGroups
      .filter(g => g.every(id => unassignedIds.has(id)))
      .map(g => ({ playerIds: g, rating: g.reduce((s,id) => s + players.find(p => p.id === id).rating, 0) }))
      .sort((a,b) => b.rating - a.rating);

    groups.forEach(g => {
      const t = womenOf('team1') < womenOf('team2') ? 'team1' : womenOf('team2') < womenOf('team1') ? 'team2' : ratingOf('team1') <= ratingOf('team2') ? 'team1' : 'team2';
    ...
    men.forEach(p => { assignments[p.id] = ratingOf('team1') <= ratingOf('team2') ? 'team1' : 'team2'; });
    ...
    const assignJerseys = (teamPlayers, inv) => {
      ...
      const assignSkater = (p) => {
        ...
        let done = false;
        for (let i = si; i < skaterSizes.length; i++) { if (inv[skaterSizes[i]] > 0) { p.assignedSize = skaterSizes[i]; inv[skaterSizes[i]]--; done = true; break; } }
    ...
    const t1p = players.filter(p => assignments[p.id] === 'team1');
    const t2p = players.filter(p => assignments[p.id] === 'team2');
    assignJerseys(t1p, t1Inv);
    assignJerseys(t2p, t2Inv);

    setPlayers(prev => prev.map(p => {
      const team = assignments[p.id] || null;
  ...
  const stats = useMemo(() => {
    ...
    [...t1,...t2].forEach(p => { if(p.assignedSize===p.preferredSize) preferredSizeMet++; else if(p.assignedSize&&p.assignedSize!=='TBD') sizedUp++; else unmetNeeds++; });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowNewSeason(!showNewSeason)} className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition"><Plus size={18} /></button>
              </div>
              {showNewSeason && (
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowNewClass(!showNewClass)} disabled={!selectedSeasonId} className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition disabled:bg-slate-300"><Plus size={18} /></button>
              </div>
              {showNewClass && (
                <div className="flex gap-2 mt-2">
          ...
          {selectedSeason && selectedClass && (
            <p className="mt-3 text-sm text-slate-500">Currently editing: <span className="font-semibold text-slate-700">{selectedSeason.name} — {selectedClass.name}</span></p>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {['team1','team2'].map(team => {
            ...
            const avg = tp.length>0?(tp.reduce((s,p)=>s+p.rating,0)/tp.length).toFixed(2):'0.00';
            return (
              <div key={team} className="bg-white rounded-lg shadow-lg p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2 bg-slate-100 px-3 py-1 rounded-lg"><Star className="text-yellow-500" size={18} /><span className="font-bold text-slate-700">{avg}</span></div>
                </div>
                <div className="space-y-2">
                  {tp.map(player => {
                    const gi = friendGroups.findIndex(g => g.includes(player.id));
                    ...
                    return (
                      <div key={player.id} className={`p-3 rounded flex justify-between items-center ${gi>=0?groupColors[gi%groupColors.length]:'bg-slate-50'} ${gi>=0?'border-2':''}`}>
                        <div>
                          <p className="font-medium">{player.name||`Player ${player.id}`}</p>
                          <p className="text-sm text-slate-600">
                            Rating: {player.rating} | Jersey: {player.preferredSize} → <span className={`font-medium ${noSize?'text-red-600':sizedUp?'text-yellow-600':'text-green-600'}`}>{assigned}</span>
                  ...
                  {tp.length===0&&<p className="text-slate-400 text-center py-4">No players assigned</p>}
                </div>
              </div>
            );
          })}
